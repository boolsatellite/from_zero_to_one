...
public:
   ...
   : minimum_size(1),
     ...
     projected_element_count(10000),                                    //要插入的数量
     false_positive_probability(1.0 / projected_element_count),         //假阳率
   ...
   unsigned long long int projected_element_count;      //要插入Bloom过滤器的大致数量
   ...
   double false_positive_probability;                   //假阳率

   unsigned long long int random_seed;                  //随机数种子

   struct optimal_parameters_t                          //最优参数结构体
   {
   ...
   virtual bool compute_optimal_parameters()            //计算最优参数
   {
      /*
        Note:
        ...
        下面将尝试找出散列函数的数量和构建 Bloom 和最小存储位数，
        以构建一个符合用户定义的误报概率和估计元素插入数的 Bloom 过滤器。
        和估计的元素插入次数相一致。
      */
...
public:
   ...
   : projected_element_count_(p.projected_element_count),                       //要插入元素的大致数目
     inserted_element_count_(0),                                                //当前插入元素的数目
     random_seed_((p.random_seed * 0xA5A5A5A5) + 1),                            //随机数种子
     desired_false_positive_probability_(p.false_positive_probability)          //假阳率
   {
      salt_count_ = p.optimal_parameters.number_of_hashes;                      //通过bloom_parameter计算最优结果中hash函数数量
      table_size_ = p.optimal_parameters.table_size;                            //通过bloom_parameter计算最优结果中bit数
   ...
   inline double effective_fpp() const
   {
      /*
        ...
        有效误报概率是使用指定的表大小和哈希函数数量，
        结合使用指定的表大小和哈希函数计数，并结合前插入元素的数量，
        而不是用户定义的 插入元素的预定/预期数量。
      */
   ...
   {
       //std::vector::data() 返回指向内存中数组第一个元素的指针
...
protected:
   ...
   void generate_unique_salt()
   {
      /*
        ...
        不同的哈希函数在实现上不一定是不同的。
        在目前的实现中，给一个通用的 散列函数 "播种 "不同的值似乎就足够了。
      */
      ...
      if (salt_count_ <= predef_salt_count)
      {
         ...
         for (std::size_t i = 0; i < salt_.size(); ++i)
         {
            ...
            */
            ...
            //random_seed_在构造函数中被传入，这样保证了每次salt的不同，增强hash函数的可变性
         }
      }
      else      //salt_count > predef_salt_count
      {
